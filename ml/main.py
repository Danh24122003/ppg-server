"""
PPG Backend Server - UNIFIED v4.3 (HeartPy + RR Accumulator + ML)
====================================================================
Cải tiến so với v3.0:
  ...
  8. ML integration: BP prediction qua EnsemblePredictor (RF + DL + Transformer)
"""
...
app = FastAPI(title="PPG Server (HRV v4.3 + ML)", version="4.3.0")
...
class PPGResult(BaseModel):
    ...
    peaks: List[int]
    timestamp: str
    ml_predictions: Optional[Dict] = None  # BP prediction từ ML model
...
# ============================================================
# ML predictor — load models on startup
# ============================================================
ensemble_predictor = EnsemblePredictor()
logger.info("ML Ensemble predictor đã khởi tạo với {} models",
...
def root():
    return {
        ...
        "features": [
            ...
            "HRV time-domain (SDNN/RMSSD/pNN50/pNN20)",
            "HRV frequency-domain (LF/HF, Welch PSD)",
            "Temporal Smoothing SpO2 (median 5)",
            "ML BP prediction (RF + DL + Transformer ensemble)",
        ],
...
def train_model(
    ...
    try:
        logger.info("Đang tạo {} mẫu synthetic...", req.num_samples)
        features_list, labels = generate_synthetic_training_data(
            num_samples=req.num_samples,
        )

        logger.info("Đang train model {}...", req.model_type)
